...
module.exports.ClusterPeer = class {
  ...
  emitToEdges(path, data){
    ...
    this.subscriptions.search(path).forEach((subscription)=>{
      ...
      edgePeer.emitClientData(subscription.clientAddress, data);
    });
  }

  emitClientData(clientAddress, data){
    console.log('emitClientData:::', data.path, clientAddress);
    this.clientConnections[clientAddress].emit(clientAddress + ':data', {path:data.path, payload:data.payload});
  }
